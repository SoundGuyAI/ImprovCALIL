...
This runs `npx prettier --check .` (Prettier) → `npm run lint` (ESLint) → `npx tsc --noEmit` (TypeScript check) → `npm test` (Unit tests) → `npm run build` (Next.js build) → `npm run test:e2e` (Playwright E2E tests) and outputs self-correcting reports in `.agents/reports/` if errors are detected.
