...
const STEPS = [
  {
    ...
    errorDescription:
      "Code formatting violations found. Run 'npx prettier --write .' to format all files.",
  },
  {
    name: "Linting",
    command: "npm",
  ...
  },
  {
    ...
    command: "npx",
    args: ["tsc", "--noEmit"],
    rules: [".cursor/rules/javascript-typescript-code-quality.mdc"],
    errorDescription: "TypeScript compilation type check failed.",
  },
  {
    name: "Unit Tests",
    command: "npm",
  ...
  },
  {
    ...
    errorDescription:
      "Next.js build compilation failed. This usually indicates TypeScript syntax, bad imports, or SSR errors.",
  },
  {
...
function formatFailuresReportMarkdown(failure) {
  const ruleLinks = failure.governingRules
    .map(
      (rule) =>
        `- [${path.basename(rule)}](file:///${path.resolve(path.join(__dirname, "..", rule))})`
    )
    .join("\n");
