...
export default function AdminConsole() {
  ...
  const [activeTab, setActiveTab] = useState<'dashboard' | 'queue' | 'events' | 'organizers' | 'simulator'>('dashboard');
  ...
  const handleToggleVisibility = async (collectionName: 'events' | 'organizers', id: string, currentHidden: boolean) => {
    try {
  ...
  const handleDelete = async (collectionName: 'events' | 'organizers', id: string) => {
    if (!confirm(locale === 'he' ? 'האם אתה בטוח שברצונך למחוק לצמיתות רשומה זו?' : 'Are you sure you want to permanently delete this record?')) return;
    try {
    ...
    setTimeout(async () => {
      ...
      try {
        const mockSubmissionData = {
          ...
          data: {
            name: simSource === 'telegram' ? "Telegram Community Workshop" : "WhatsApp Scraped Show Match",
            ...
            recurrence: 'one-time',
            location: simSource === 'telegram' ? "Community Hub, Ra'anana" : "Improv Club, Tel Aviv",
  ...
  const filteredEvents = events.filter(e => {
    ...
    if (searchQuery && !e.name.toLowerCase().includes(searchQuery.toLowerCase()) && !e.organizerName.toLowerCase().includes(searchQuery.toLowerCase())) return false;
    return true;
  });
    ...
    <div className="min-h-screen bg-zinc-950 text-zinc-100 flex flex-col">
      ...
      <main className="flex-grow max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-6 flex flex-col gap-6">
        ...
        <div className="flex flex-col gap-2">
          ...
          <p className="text-zinc-400 text-sm">{locale === 'he' ? 'נהלו את לוח האירועים, אינדקס המארגנים ואשרו הגשות חדשות מהקהילה.' : 'Moderate community calendars, reference indexes, and review crowd-sourced submissions.'}</p>
        </div>
          ...
          <div className="flex flex-col gap-6">
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
              ...
              <div className="glass-card rounded-2xl p-6 border border-zinc-900 flex flex-col gap-2">
                <span className="text-zinc-500 text-xs font-bold uppercase">{tAdmin('pendingSubmissions')}</span>
                <span className="text-4xl font-extrabold text-indigo-400">{pendingSubsCount}</span>
              </div>
              
              {/* Stat 2 */}
              <div className="glass-card rounded-2xl p-6 border border-zinc-900 flex flex-col gap-2">
                <span className="text-zinc-500 text-xs font-bold uppercase">{tAdmin('activeEvents')}</span>
                <span className="text-4xl font-extrabold text-emerald-400">{events.filter(e => !e.hidden).length}</span>
              </div>

              {/* Stat 3 */}
              <div className="glass-card rounded-2xl p-6 border border-zinc-900 flex flex-col gap-2">
                <span className="text-zinc-500 text-xs font-bold uppercase">{tAdmin('totalOrganizers')}</span>
            ...
            <div className="glass-card rounded-2xl p-6 border border-zinc-900 flex flex-col gap-3">
              <h3 className="text-md font-bold text-white uppercase">{locale === 'he' ? 'צינור קליטה מבוסס AI' : 'AI Ingestion Pipeline Overview'}</h3>
            ...
            {submissions.filter(s => s.status === 'pending').length === 0 ? (
              ...
              <div className="flex flex-col gap-4">
                {submissions.filter(s => s.status === 'pending').map((sub) => (
                  <div key={sub.id} className="glass-card rounded-xl p-5 border border-zinc-900 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                    <div className="flex flex-col gap-2">
                      ...
                      <div className="flex flex-wrap items-center gap-4 text-[10px] text-zinc-500 font-bold mt-1">
                        ...
                        {sub.data.time && <span>Time: {new Date(sub.data.time).toLocaleString()}</span>}
                        {sub.submitterContact?.email && <span>Submitter: {sub.submitterContact.email}</span>}
                      </div>
                    </div>
            ...
            <div className="overflow-x-auto w-full border border-zinc-900 rounded-xl bg-zinc-950/40">
              <table className="w-full text-sm text-left rtl:text-right text-zinc-400">
                <thead className="text-xs uppercase bg-zinc-950 text-zinc-500 border-b border-zinc-900">
                  <tr>
                    <th scope="col" className="px-6 py-4 font-bold">{locale === 'he' ? 'אירוע' : 'Event'}</th>
                    <th scope="col" className="px-6 py-4 font-bold">{locale === 'he' ? 'מארגן' : 'Organizer'}</th>
                    <th scope="col" className="px-6 py-4 font-bold">{locale === 'he' ? 'אזור' : 'Region'}</th>
                ...
                <tbody className="divide-y divide-zinc-900">
                  {filteredEvents.map((evt) => (
                    <tr key={evt.id} className={`hover:bg-zinc-900/30 ${evt.hidden ? 'opacity-50 bg-zinc-950/20' : ''}`}>
                      <td className="px-6 py-4 font-semibold text-white">
                        <div className="flex flex-col">
                          <span>{evt.name}</span>
                          <span className="text-[10px] text-zinc-600 font-bold uppercase">{evt.id}</span>
                        </div>
                      </td>
                      ...
                      <td className="px-6 py-4 text-xs">
                        <div className="flex gap-1.5 flex-wrap">
                          {evt.hidden && <span className="px-1.5 py-0.5 rounded bg-zinc-900 text-[10px] text-zinc-500 font-extrabold uppercase border border-zinc-800">{tAdmin('hiddenBadge')}</span>}
                          {evt.featured && <span className="px-1.5 py-0.5 rounded bg-indigo-500/10 text-[10px] text-indigo-400 font-extrabold uppercase border border-indigo-500/20">{tAdmin('featuredBadge')}</span>}
                        </div>
                      </td>
                      <td className="px-6 py-4 text-xs">
                        <div className="flex items-center gap-2">
                          ...
                          >
                            {evt.hidden ? <Eye className="w-4 h-4 text-indigo-400" /> : <EyeOff className="w-4 h-4 text-zinc-500" />}
                          </button>
                          ...
                          >
                            <Sparkles className={`w-4 h-4 ${evt.featured ? 'text-amber-400' : 'text-zinc-500'}`} />
                          </button>
            ...
            <div className="overflow-x-auto w-full border border-zinc-900 rounded-xl bg-zinc-950/40">
              <table className="w-full text-sm text-left rtl:text-right text-zinc-400">
                <thead className="text-xs uppercase bg-zinc-950 text-zinc-500 border-b border-zinc-900">
                  <tr>
                    <th scope="col" className="px-6 py-4 font-bold">{locale === 'he' ? 'מארגן' : 'Organizer'}</th>
                    <th scope="col" className="px-6 py-4 font-bold">{locale === 'he' ? 'סוג' : 'Type'}</th>
                    <th scope="col" className="px-6 py-4 font-bold">{locale === 'he' ? 'אזור' : 'Region'}</th>
                    <th scope="col" className="px-6 py-4 font-bold">Status</th>
                    <th scope="col" className="px-6 py-4 font-bold">Actions</th>
                  </tr>
                ...
                <tbody className="divide-y divide-zinc-900">
                  {filteredOrganizers.map((org) => (
                    <tr key={org.id} className={`hover:bg-zinc-900/30 ${org.hidden ? 'opacity-50 bg-zinc-950/20' : ''}`}>
                      <td className="px-6 py-4 font-semibold text-white">
                        <div className="flex flex-col">
                          <span>{org.name}</span>
                          <span className="text-[10px] text-zinc-600 font-bold uppercase">{org.id}</span>
                        </div>
                      </td>
                      ...
                      <td className="px-6 py-4 text-xs">
                        <div className="flex gap-1.5 flex-wrap">
                          {org.hidden && <span className="px-1.5 py-0.5 rounded bg-zinc-900 text-[10px] text-zinc-500 font-extrabold uppercase border border-zinc-800">{tAdmin('hiddenBadge')}</span>}
                          <span className="px-1.5 py-0.5 rounded bg-emerald-500/10 text-[10px] text-emerald-400 font-extrabold uppercase border border-emerald-500/20">{org.publishStatus}</span>
                        </div>
                      </td>
                      <td className="px-6 py-4 text-xs">
                        <div className="flex items-center gap-2">
                          ...
                          >
                            {org.hidden ? <Eye className="w-4 h-4 text-indigo-400" /> : <EyeOff className="w-4 h-4 text-zinc-500" />}
                          </button>
          ...
          <div className="glass-card rounded-2xl p-6 flex flex-col gap-6">
            ...
            {simSuccess && (
              <div className="p-4 rounded-xl border border-emerald-500/25 bg-emerald-500/5 text-emerald-400 text-sm font-bold flex items-center gap-2">
                ...
                <span>{locale === 'he' ? 'סימולציית הקליטה הצליחה! טיוטת האירוע התווספה לתור האישורים.' : 'Simulation successful! Parsed draft event has been added to the moderation queue.'}</span>
              </div>
            )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-start">
              ...
              <div className="flex flex-col gap-4 bg-zinc-950/40 p-4 rounded-xl border border-zinc-900">
                <div className="flex flex-col gap-1.5">
                  <label className="text-xs text-zinc-500 font-bold uppercase">{locale === 'he' ? 'מקור קליטה' : 'Ingestion Source'}</label>
                ...
                <div className="flex flex-col gap-2.5 mt-2">
                  <span className="text-xs text-zinc-500 font-bold uppercase">{locale === 'he' ? 'דוגמאות הודעה מהירות' : 'Quick Message Presets'}</span>
                  
                  <button
                    onClick={() => setSimText(simSource === 'telegram' 
              ...
              <div className="md:col-span-2 flex flex-col gap-4">
                <label className="text-xs font-extrabold uppercase text-zinc-400">
                  {simSource === 'telegram' ? tAdmin('simulateTelegram') : tAdmin('simulateWhatsApp')}
                </label>
                ...
                <textarea
                  ...
                  placeholder={locale === 'he' ? 'הדביקו הודעת וואטסאפ או טקסט שנשלח לבוט...' : 'Paste message payload to parse...'}
