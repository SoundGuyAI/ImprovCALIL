...
export default function SubmitContent() {
  ...
  const handleAiParse = () => {
    ...
    setTimeout(() => {
      ...
      else if (lower.includes("workshop") || lower.includes("סדנה") || lower.includes("קורס")) name = "Improv Scene Relationships Workshop";

      let cost = "Paid";
      if (lower.includes("free") || lower.includes("חינם") || lower.includes("כניסה חופשית")) cost = "Free";
    ...
    <div className="min-h-screen bg-zinc-950 text-zinc-100 flex flex-col">
      ...
      <main className="flex-grow max-w-4xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-6 flex flex-col gap-6">
        ...
        <div className="flex flex-col gap-2">
          ...
          <p className="text-zinc-400 text-sm">{locale === 'he' ? 'הגישו אירועים או מארגנים חדשים ללוח. התכנים יפורסמו באופן מיידי לאחר בקרה ואישור מהירים.' : 'Submit upcoming events or listing proposals. Content will be published immediately after quick admin moderation.'}</p>
        </div>

        {/* TABS NAVBAR */}
        <div className="flex border-b border-zinc-900 bg-zinc-950 sticky top-16 z-10 py-2 gap-2">
          <button
            onClick={() => { setActiveTab('event'); setSuccess(false); setError(false); }}
          ...
          <button
            onClick={() => { setActiveTab('organizer'); setSuccess(false); setError(false); }}
            className={`flex items-center gap-2 px-4 py-2.5 rounded-lg text-sm font-bold transition-all cursor-pointer ${
              activeTab === 'organizer' 
                ? 'bg-zinc-800 text-white shadow-md' 
                : 'text-zinc-500 hover:text-zinc-300 hover:bg-zinc-900/50'
          ...
          <button
            onClick={() => { setActiveTab('ai'); setSuccess(false); setError(false); }}
        ...
        {activeTab === 'event' && !success && (
          <form onSubmit={handleEventSubmit} className="glass-card rounded-2xl p-6 flex flex-col gap-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {/* Event Name */}
              <div className="flex flex-col gap-2">
                <label className="text-xs font-extrabold uppercase text-zinc-400">{tSub('eventName')} *</label>
                <input
                  type="text"
                  required
                  value={eventName}
              ...
              {/* Region */}
              <div className="flex flex-col gap-2">
                <label className="text-xs font-extrabold uppercase text-zinc-400">{locale === 'he' ? 'אזור בארץ' : 'Region'} *</label>
                <select
                  value={eventRegion}
                ...
                >
                  {Object.keys(tRegions.messages).map((key) => (
              ...
              {/* Start Date & Time */}
              <div className="flex flex-col gap-2">
                <label className="text-xs font-extrabold uppercase text-zinc-400">{tSub('eventTime')} *</label>
                <input
                  type="datetime-local"
                  required
                  value={eventTime}
              ...
              {/* End Date & Time */}
              <div className="flex flex-col gap-2">
                <label className="text-xs font-extrabold uppercase text-zinc-400">{tSub('eventEndTime')}</label>
                <input
                  type="datetime-local"
                  value={eventEndTime}
              ...
              <div className="flex flex-col gap-2 md:col-span-2">
                <label className="text-xs font-extrabold uppercase text-zinc-400">{tSub('location')} *</label>
                <input
                  type="text"
                  required
                  value={eventLocation}
              ...
              {/* Map Link */}
              <div className="flex flex-col gap-2 md:col-span-2">
                <label className="text-xs font-extrabold uppercase text-zinc-400">{tSub('mapLink')}</label>
                <input
                  type="url"
                  value={eventMapLink}
              ...
              <div className="flex flex-col gap-2">
                <label className="text-xs font-extrabold uppercase text-zinc-400">{tSub('organizerSelect')}</label>
                <select
                  value={eventOrganizerId}
                  onChange={(e) => setEventOrganizerId(e.target.value)}
                  className="px-4 py-2.5 rounded-xl border border-zinc-800 bg-zinc-950/60 text-zinc-300 focus:outline-none focus:border-indigo-500 text-sm"
                >
                  <option value="">{locale === 'he' ? '-- בחר מארגן מהאינדקס --' : '-- Choose from Directory --'}</option>
                  {organizers.map(org => (
              ...
              <div className="flex flex-col gap-2">
                <div className="flex flex-col">
                  <label className="text-xs font-extrabold uppercase text-zinc-400">{tSub('organizerString')}</label>
                ...
                <input
                  ...
                  placeholder={eventOrganizerId ? "Locked (Linked to Directory)" : "e.g. Living Room Jam Team"}
              ...
              <div className="flex flex-col gap-2">
                <label className="text-xs font-extrabold uppercase text-zinc-400">{tSub('language')} *</label>
                <select
                  value={eventLanguage}
              ...
              <div className="flex flex-col gap-2">
                <label className="text-xs font-extrabold uppercase text-zinc-400">{tSub('cost')} *</label>
                <select
                  value={eventCost}
              ...
              <div className="flex flex-col gap-2">
                <label className="text-xs font-extrabold uppercase text-zinc-400">{tSub('access')} *</label>
                <select
                  value={eventAccess}
              ...
              <div className="flex flex-col gap-2">
                <label className="text-xs font-extrabold uppercase text-zinc-400">{locale === 'he' ? 'מחזוריות' : 'Recurrence'}</label>
                <select
              ...
              {/* Description */}
              <div className="flex flex-col gap-2 md:col-span-2">
                <label className="text-xs font-extrabold uppercase text-zinc-400">{tSub('description')} *</label>
                <textarea
                  required
                  rows={4}
            ...
            <div className="flex flex-col gap-4 border-t border-zinc-900 pt-4">
              <div className="flex justify-between items-center">
                <label className="text-xs font-extrabold uppercase text-zinc-400">{tSub('linksSection')}</label>
                <button
                  type="button"
              ...
              {eventLinks.map((lnk, idx) => (
                <div key={idx} className="grid grid-cols-1 sm:grid-cols-3 gap-3 items-center bg-zinc-950/40 p-3 rounded-xl border border-zinc-900">
                  <select
                    value={lnk.type}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 border-t border-zinc-900 pt-6">
              <div className="flex flex-col gap-2">
                <label className="text-xs font-extrabold uppercase text-zinc-400">{tSub('contactEmail')} *</label>
              ...
              <div className="flex flex-col gap-2">
                <label className="text-xs font-extrabold uppercase text-zinc-400">{tSub('contactPhone')}</label>
                <div className="relative">
        ...
        {activeTab === 'organizer' && !success && (
          <form onSubmit={handleOrganizerSubmit} className="glass-card rounded-2xl p-6 flex flex-col gap-6">
            <div className="border-b border-zinc-850 pb-3">
              <h2 className="text-lg font-bold text-white">{tSub('organizerFormDesc')}</h2>
              <p className="text-xs text-zinc-400 mt-1">{locale === 'he' ? 'הוסיפו גוף אימפרוב לאינדקס למטרות הפנייה.' : 'Propose an entry or listing update.'}</p>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {/* Proposal Type */}
              <div className="flex flex-col gap-2 md:col-span-2">
                <label className="text-xs font-extrabold uppercase text-zinc-400">{tSub('newVsUpdate')}</label>
              ...
              {/* Name */}
              <div className="flex flex-col gap-2">
                <label className="text-xs font-extrabold uppercase text-zinc-400">{tSub('organizerName')} *</label>
                <input
                  type="text"
                  required
                  value={orgName}
              ...
              {/* Type */}
              <div className="flex flex-col gap-2">
                <label className="text-xs font-extrabold uppercase text-zinc-400">{tSub('organizerType')} *</label>
                <select
                  value={orgType}
                  onChange={(e) => setOrgType(e.target.value)}
                  className="px-4 py-2.5 rounded-xl border border-zinc-800 bg-zinc-950/60 text-zinc-300 focus:outline-none focus:border-indigo-500 text-sm"
                >
                  {Object.keys(tOrgTypes.messages).map((key) => (
              ...
              {/* Region */}
              <div className="flex flex-col gap-2">
                <label className="text-xs font-extrabold uppercase text-zinc-400">{locale === 'he' ? 'אזור מאגר' : 'Region'} *</label>
                <select
                  value={orgRegion}
              ...
              {/* Languages Supported */}
              <div className="flex flex-col gap-2">
                <label className="text-xs font-extrabold uppercase text-zinc-400">{locale === 'he' ? 'שפות פעילות' : 'Languages'}</label>
        ...
        {activeTab === 'ai' && (
          <div className="glass-card rounded-2xl p-6 flex flex-col gap-6">
            ...
            <div className="flex flex-col gap-2.5 border-t border-zinc-900 pt-4 mt-2">
              <span className="text-xs text-zinc-500 font-extrabold uppercase">{locale === 'he' ? 'מודעות לדוגמה (לחצו להדבקה מהירה)' : 'Mock Flyers (Click to auto-paste)'}</span>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <button
                  onClick={() => setFlyerText("🔥 COMMUNITY IMPROV JAM IN JERUSALEM! 🔥\nEvery Tuesday at 20:00! Join us at the Gerard Behar Center. Completely free stage, open for all experience levels to play and laugh. English speakers friendly!")}
