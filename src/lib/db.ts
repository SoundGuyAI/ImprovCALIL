...
export async function getEvents(filters?: {
  ...
  try {
    ...
    for (const docOfSnap of snap.docs) {
      ...
      if (filters?.type && filters.type !== 'all' && data.recurrence !== filters.type && data.type !== filters.type) {
        // Handle "type" checks (could be one-time/weekly or Show/Jam categories)
        // Check both recurrence and custom properties
      }
      if (filters?.language && filters.language !== 'all' && data.language !== filters.language) continue;
...
export async function createSubmission(submission: Omit<FirestoreSubmission, 'id' | 'createdAt' | 'status'>): Promise<string> {
  try {
...
export async function updateRecordVisibility(collectionName: 'events' | 'organizers', id: string, hidden: boolean): Promise<void> {
  try {
...
// 11. Delete Record
export async function deleteRecord(collectionName: 'events' | 'organizers', id: string): Promise<void> {
  try {
